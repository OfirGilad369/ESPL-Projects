...
int addCmdLine(shellHistoryArray * sHistoryArray, cmdLine *pCmdLine) {
	...
	if (sHistoryArray->isFull == 0) {
		sHistoryArray->history_array[sHistoryArray->lastIndex + 1] = copy_cmd_line(pCmdLine);
	...
	freeCmdLines(sHistoryArray->history_array[currentIndex]);
	sHistoryArray->history_array[currentIndex] = copy_cmd_line(pCmdLine);
...
void printHistory(shellHistoryArray *sHistoryArray) {
	char cmd_line_string[2048];

	if (sHistoryArray->isFull == 0) {
		int size_of_history = sHistoryArray->lastIndex + 1;
		int i;
		for(i = 0; i < size_of_history; i++) {
			cmdLineToString(sHistoryArray->history_array[i], cmd_line_string);
			printf(" %d  %s\n", i, cmd_line_string);
		}
	} else {
		int j;
...
void freeHistory(shellHistoryArray *sHistoryArray) {
	...
	for(i = 0; i < MAX_HISTORY_SIZE; i++) {
		freeCmdLines(sHistoryArray->history_array[i]);
	}
	FREE(sHistoryArray);
}
