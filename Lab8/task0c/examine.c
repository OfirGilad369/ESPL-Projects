...
int main(int argc, char **argv) {
	...
	print_elf_file_data_encoding_format(&elf_file_header);
	print_elf_file_entry_point(&elf_file_header);
	print_elf_file_section_header_table_offset(&elf_file_header);
    print_elf_file_section_header_entries_number(&elf_file_header);
    
    Elf32_Shdr *elf_file_section_headers = load_elf_file_section_headers(&elf_file_header, mapped_elf_file);
    
    print_elf_file_section_header_entries_sizes(elf_file_section_headers, elf_file_header.e_shnum);
    print_shstrtab_index(&elf_file_header);
